...
import static org.mitre.disttree.stores.DataStores.duckDbStore;
...
public class DuckDBTreeTest {
    ...
    // Something like: "/var/folders/1p/8vfyvr2j6_s9r0typ1ld69400000gp/T/junit5400551587074358402"
    @TempDir
    File testDir;
    ...
    public void simplestPossibleSpinUp_rootNeverSplits() {
        ...
        TreeConfig<LatLong, String> config = TreeConfig.<LatLong, String>builder()
                .maxTuplesPerPage(64)
                .distMetric(METRIC)
                .dataStore(duckDbStore(testDir.getAbsolutePath()))
                .keySerde(KEY_SERDE)
                .valueSerde(VALUE_SERDE)
                .build();

        InternalTree<LatLong, String> tree = new InternalTree<>(config);
    ...
    @Test
    public void testLargeDuckDBTree() {
        ...
        // put all testData in the tree (large batches are much faster)
        List<Batch<LatLong, String>> batches = batchify(testData, 1_000);
    ...
    public void testTreeWithNullValues() {
        ...
        // Here we build a DistanceTree where we never provide the "Value V" Strings
